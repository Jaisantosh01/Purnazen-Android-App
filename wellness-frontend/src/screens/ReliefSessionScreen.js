...
import { RELIEF_SESSIONS } from '../data/reliefSessionData';
import reliefService from '../services/reliefService';
import therapyService from '../services/therapyService';

const ReliefSessionScreen = ({ navigation, route }) => {
  const reliefKey = route?.params?.reliefKey || 'Headache';
  const SESSION = RELIEF_SESSIONS[reliefKey] || RELIEF_SESSIONS['Headache'];
  ...
  useEffect(() => {
    const fetchSession = async () => {
      try {
        const data = await reliefService.getReliefSession(reliefKey);
        setSessionData(data);
      } catch (err) {
        console.log('Session fetch failed, using local data:', err.message);
      }
    };
    fetchSession();
  }, [reliefKey]);
    ...
    if (isPlaying) {
      timerRef.current = setInterval(() => {
        setTimeLeft(prev => {
          if (prev <= 1) {
            handleStepComplete();
            return steps[currentStep]?.duration || 30;
          }
          return prev - 1;
        });
      }, 1000);
  ...
  const handleStepComplete = useCallback(() => {
    ...
    if (currentStep < totalSteps - 1) {
      ...
      if (currentCycle < activeSession.totalCycles) {
        setCurrentCycle(prev => prev + 1);
        setCurrentStep(0);
        setCompletedSteps([]);
        setTimeLeft(activeSession.steps[0].duration);
  ...
  }, [currentStep, totalSteps, steps, currentCycle, activeSession, progressAnim]);
    ...
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSubtitle}>{activeSession.duration} session</Text>
        </View>
        <Text style={styles.cycleText}>
          Cycle {currentCycle}/{activeSession.totalCycles}
        </Text>
      </View>
        ...
        <View style={styles.animationArea}>
          {activeSession.videoUrl ? (
            <Video
              ...
              resizeMode="contain"
              repeat={false}
              onEnd={handleStepComplete}
            />
          ) : (
            <View style={styles.iconCircle}>
              <Text style={styles.poseIcon}>{activeSession.icon}</Text>
            </View>
          )}
          <TouchableOpacity
            ...
            return (
              ...
              >
                ...
                <View style={styles.stepRowInfo}>
                  <Text style={[styles.stepRowName, isActive && styles.stepRowNameActive]}>
                    {step.name}
                  </Text>
      ...
      <View style={styles.bottomBar}>
        ...
        <TouchableOpacity style={styles.startBtn} onPress={handlePlayPause} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  cycleText: { fontSize: 13, fontWeight: '600', color: '#1FA77A' },
  ...
  floatingPlayIcon: { fontSize: 18, color: '#fff' },
  ...
  stepCount: { fontSize: 14, fontWeight: '600', color: '#1a1a1a' },
  timerText: { fontSize: 18, fontWeight: '700', color: '#1FA77A' },
  progressBarBg: {
    height: 6,
  ...
  progressBarFill: { height: '100%', backgroundColor: '#1FA77A', borderRadius: 3 },
  dotsRow: { flexDirection: 'row', gap: 6 },
  dot: { width: 8, height: 8, borderRadius: 4, backgroundColor: 'rgba(31,167,122,0.3)' },
  dotActive: { backgroundColor: '#1FA77A', width: 10, height: 10, borderRadius: 5 },
  dotCompleted: { backgroundColor: '#1FA77A' },
  ...
  stepNumber: { fontSize: 15, fontWeight: '700', color: '#1FA77A' },
  stepInfo: { flex: 1 },
  stepName: { fontSize: 15, fontWeight: '700', color: '#1a1a1a', marginBottom: 4 },
  stepDescription: { fontSize: 13, color: '#6b7280' },

  sessionStepsSection: { marginHorizontal: 16, marginTop: 20 },
  sessionStepsLabel: {
    fontSize: 11,
    fontWeight: '700',
    color: '#9ca3af',
  ...
  stepRowActive: { backgroundColor: '#e8f8f2', borderWidth: 1, borderColor: '#1FA77A' },
  stepRowNumber: {
    width: 30,
    height: 30,
    borderRadius: 15,
  ...
  stepRowNumberActive: { backgroundColor: '#1FA77A' },
  stepRowNumberCompleted: { backgroundColor: '#1FA77A' },
  stepRowNumberText: { fontSize: 13, fontWeight: '600', color: '#6b7280' },
  stepRowNumberTextActive: { color: '#fff' },
  checkIcon: { fontSize: 14, color: '#fff', fontWeight: '700' },
  stepRowInfo: { flex: 1 },
  stepRowName: { fontSize: 14, fontWeight: '600', color: '#1a1a1a' },
  stepRowNameActive: { color: '#1FA77A' },
  stepRowDuration: { fontSize: 12, color: '#9ca3af', marginTop: 2 },
  ...
  restartIcon: { fontSize: 16, color: '#6b7280' },
  restartText: { fontSize: 15, fontWeight: '600', color: '#6b7280' },
  startBtn: {
    flex: 2,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  startBtnIcon: { fontSize: 16, color: '#fff' },
