...
const PrescriptionEditorScreen = ({ route, navigation }) => {
  ...
  const records = useConsultationStore(s => s.prescriptions);
  const addRecord = useConsultationStore(s => s.addPrescription);
  const updateRecord = useConsultationStore(s => s.updatePrescription);
  ...
  return (
    <View style={styles.root}>
      <ScreenHeader
        title={mode === 'edit' ? 'Edit Prescription' : 'Prescription'}
...
export default PrescriptionEditorScreen;
