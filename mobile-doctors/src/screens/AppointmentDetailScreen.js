...
const AppointmentDetailScreen = ({ route, navigation }) => {
  ...
  const handleUpdateStatus = async (status) => {
    try {
      // 1. Optimistically update local details state
      setAppointment(prev => prev ? { ...prev, status } : null);

      // 2. Optimistically update parent list screen state
      if (route.params?.onStatusUpdate) {
        route.params.onStatusUpdate(id, status);
      }

      // 3. Call backend API to persist
      await appointmentService.updateStatus(id, status);

      // 4. Load fresh details to fully sync
      const freshData = await appointmentService.detail(id);
      if (freshData) {
